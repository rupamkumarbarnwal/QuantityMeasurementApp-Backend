...
import quantityMeasurement.model.QuantityModel;
...
public class QuantityMeasurementServiceImpl implements IQuantityMeasurementService {
    ...
    private QuantityModel<IMeasurable> getQuantityModel(QuantityDTO quantity) {
        ...
        if (quantity.getUnitName() == null || quantity.getMeasurementType() == null)
            throw new IllegalArgumentException("Quantity fields cannot be null");
    ...
    private <U extends IMeasurable> void validateArithmeticOperands(
        ...
        if (thisModel.getUnit().getClass() != thatModel.getUnit().getClass())
            throw new IllegalArgumentException(
        ...
        if (targetUnitRequired && targetUnitModel == null)
    ...
    private <U extends IMeasurable> double performArithmetic(
        ...
        thisModel.getUnit().validateOperationSupport(operation.name());
    ...
    private <U extends IMeasurable> double convertTemperatureUnit(
            QuantityModel<U> thisModel,
            QuantityModel<U> thatModel) {
        TemperatureUnit srcUnit    = (TemperatureUnit) thisModel.getUnit();
        TemperatureUnit targetUnit = (TemperatureUnit) thatModel.getUnit();
        return srcUnit.convertTo(thisModel.getValue(), targetUnit);
    }

    private <U extends IMeasurable> double convertTo(
            QuantityModel<U> srcModel,
            QuantityModel<U> targetModel) {
        double base = srcModel.getUnit().convertToBaseUnit(srcModel.getValue());
        return targetModel.getUnit().convertFromBaseUnit(base);
    }
    ...
    private <U extends IMeasurable> boolean compare(
        ...
        boolean result = Math.abs(base1 - base2) < 1e-6;
    ...
    public QuantityDTO convert(QuantityDTO thisQuantityDTO,
        ...
        try {
            ...
            if (srcModel.getUnit() instanceof TemperatureUnit) {
                convertedValue = convertTemperatureUnit(srcModel, targetModel);
            } else {
                convertedValue = convertTo(srcModel, targetModel);
            }

            QuantityModel<IMeasurable> resultModel =
                    new QuantityModel<>(convertedValue, targetModel.getUnit());
    ...
    public QuantityDTO add(QuantityDTO thisQuantityDTO,
        ...
        try {
            ...
            double resultValue = target.getUnit().convertFromBaseUnit(baseResult);

            QuantityModel<IMeasurable> resultModel =
                    new QuantityModel<>(resultValue, target.getUnit());
    ...
    public QuantityDTO subtract(QuantityDTO thisQuantityDTO,
        ...
        try {
            ...
            double baseResult  =
                    performArithmetic(m1, m2, ArithmeticOperation.SUBTRACT);
            double resultValue =
                    target.getUnit().convertFromBaseUnit(baseResult);
    ...
    public double divide(QuantityDTO thisQuantityDTO,
        ...
        try {
            ...
            double result =
                    performArithmetic(m1, m2, ArithmeticOperation.DIVIDE);
    ...
    public static void main(String[] args) {
        System.out.println("QuantityMeasurementServiceImpl");
    }
}
